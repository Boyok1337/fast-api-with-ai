...
from post.ai import HARM_PROBABILITY, MODEL
...
def create_comment(
    db: Session, comment: schemas.CommentCreate, post_id: int, parent_id: int = None
):
...
def get_comments_daily_breakdown(db: Session, date_from: date, date_to: date):
    result = (
        db.query(
            ...
            func.sum(func.cast(Comment.blocked, Integer)).label("blocked_comments"),
        )
